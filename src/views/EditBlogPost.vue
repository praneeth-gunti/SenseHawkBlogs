...
export default {
    ...
    async mounted(){
        ...
        this.currentBlog = await this.$store.state.userBlogCards.filter((post) => {
            return post.blogId === this.$route.params.blogId;
        });
        if(this.currentBlog.length == 0){
            this.currentBlog = await this.$store.state.sampleBlogCards.filter((post) => {
                return post.blogId === this.$route.params.blogId;
            });            
        }
        ...
        updateBlog(){
            if(this.blogTitle.length !== 0 && this.blogHTML.length !== 0){
                if(this.file){
                    ...
                    this.$store.commit("coverPhotoChange", post);
                ...
                this.$store.commit("ChangeTitleContent", postTitle);
                this.$router.push({name: 'ViewBlog', params: {blogId: postTitle.blogId} })
            }
            this.error = true;
